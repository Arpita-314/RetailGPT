...
def create_microcoil_arm(angle):
    ...
    oEditor.CreatePolyline(
        ...
        [
            "NAME:Attributes",
            "Name:=", "MicrocoilArm_{0}".format(angle),
            "Flags:=", "",
            "Color:=", "(255 215 0)",
            "Transparency:=", 0,
            "PartCoordinateSystem:=", "Global",
    ...
    oEditor.SweepAlongVector(
        [
            "NAME:Selections",
            "Selections:=", "MicrocoilArm_{0}".format(angle),
...
oEditor.Unite(
    [
        "NAME:Selections",
        "Selections:=", ",".join(["MicrocoilArm_{0}".format(i*math.pi/4) for i in range(8)])
    ],
    [
...
for i in range(1, 9):  # 8 ports
    oModule.AssignWavePort(
        [
            "NAME:WavePort{0}".format(i),
            "Objects:=", ["WavePort{0}".format(i)],
            "NumModes:=", 1,
